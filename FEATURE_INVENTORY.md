...
- **Dynasty Perks Editor**: Opens the dynasty perk editor: per-perk add/remove toggles for all 105 vanilla perks across 21 legacy tracks (left click add, right click remove), per-track add-all/remove-all buttons, renown-free mode, a splendor-level editor, and editing of any dynasty via a picked character. The grid and its effects are generated by `tools/generate_perk_editor.ps1`.
